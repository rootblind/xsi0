...
void joc_pve(char joc[5][5])
{
    ...
    do{
        system("cls");
        afisare(joc);
        // aceleasi lucruri ca si la pvp, doar ca jucatorul 2 este jucata de functia rand(), inpropriu spus
        do{
            if(jucator  == 1)
            {
                ...
                mutare = rand() % 10;// o valoare random de la 0 la 9
        ...
        }while(mutare < 1 || mutare > 9 || joc[pi][pj] != '#');// daca rand() va fi o valoare invalida, va alege alta pana va gasi una valida
        liber--;
        if(jucator == 1)
        {
            joc[pi][pj] = 'x';
